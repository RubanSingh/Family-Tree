...
public class FamilyController extends DetailController {
    ...
    @Override
    public void format() {
        getToolbar().setTitle(R.string.family);
    ...
    public static void chooseLineage(Context context, Person person, Family family) {
        ...
        if (parentFamilyRef != null) {
            int actual = Arrays.asList(lineageTypes).indexOf(parentFamilyRef.getRelationshipType());
            new MaterialAlertDialogBuilder(context).setSingleChoiceItems(lineageTexts, actual, (dialog, i) -> {
    ...
    public static void connect(Person person, Family family, int role) {
        switch (role) {
            case 5: // Parent // TODO: code smell: use of magic number
                ...
                PersonEditorController.addSpouse(family, sr);
    ...
    protected void onResume() {
        super.onResume();
        AnalyticsUtil.logEventFamilyActivity(getFirebaseAnalytics());
    }
}
