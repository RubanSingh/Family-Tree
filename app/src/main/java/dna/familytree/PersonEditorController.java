...
public class PersonEditorController extends BaseController {
    ...
    protected void onResume() {
        super.onResume();
        AnalyticsUtil.logEventPersonEditorActivity(getFirebaseAnalytics());
    }
    ...
    void save() {
        ...
        for (EventFact fact : p.getEventsFacts()) {
            if (fact.getTag().equals("BIRT")) {
                ...
                fact.setDate(date);
                fact.setPlace(place);
                EventController.cleanUpTag(fact);
                found = true;
            }
        }
        ...
        if (!found && (!date.isEmpty() || !place.isEmpty())) {
            ...
            EventController.cleanUpTag(birth);
        ...
        if (!found && isDeadSwitch.isChecked()) {
            ...
            EventController.cleanUpTag(morte);
        ...
        if (idIndi.equals("TIZIO_NUOVO") || relation > 0) {
            ...
            if (relation >= 5) { // Comes from FamilyActivity
                FamilyController.connect(p, gc.getFamily(familyId), relation);
