...
public class NewTreeController extends BaseController {
    ...
    private static String TAG = NewTreeController.class.getSimpleName();
...
//        else
        ...
        emptyTree.setOnClickListener(v -> {
            AnalyticsUtil.logEventCreateTree(getFirebaseAnalytics());
        ...
        downloadExample.setOnClickListener(v -> {
            AnalyticsUtil.logEventDownloadTree(getFirebaseAnalytics());
            downloadExample();
        ...
        importGedcom.setOnClickListener(v -> {
            AnalyticsUtil.logEventImportGEDCOM(getFirebaseAnalytics());
        ...
        recoverBackup.setOnClickListener(v -> {
            AnalyticsUtil.logEventRecoverBackup(getFirebaseAnalytics());
            Intent intent = new Intent(Intent.ACTION_GET_CONTENT);
    ...
    static boolean unZip(Context context, String zipPath, Uri zipUri) {
        ...
        try {
            ...
            // The download happened from the referrer dialog in TreesActivity
            if (context instanceof TreesController) {
                TreesController treesPage = (TreesController)context;
    ...
    protected void onResume() {
        super.onResume();
        AnalyticsUtil.logEventNewTreeActivity(getFirebaseAnalytics());
    }
        ...
        if (resultCode == RESULT_OK && requestCode == 630) {
            try {
                ...
                if (!gedcom.getSources().isEmpty() && !Global.settings.expert) {
                    new MaterialAlertDialogBuilder(this).setMessage(R.string.complex_tree_advanced_tools)
        ...
        if (resultCode == RESULT_OK && requestCode == 219) {
            try {
                ...
                if (settingsFileExists) {
                    ...
                    Toast.makeText(NewTreeController.this, R.string.backup_invalid, Toast.LENGTH_LONG).show();
            } catch (Exception e) {
                LoggerUtils.ErrorLog(TAG, "Exception in onActivityResult", e);
                Toast.makeText(NewTreeController.this, e.getLocalizedMessage(), Toast.LENGTH_LONG).show();
            }
        }
    }
    ...
    public static Header createHeader(String nomeFile) {
        ...
        app.setValue("FAMILY_TREE");
        app.setName("Family Tree");
