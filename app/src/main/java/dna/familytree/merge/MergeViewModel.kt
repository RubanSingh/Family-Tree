...
import dna.familytree.visitor.MediaContainersGuarded
import dna.familytree.visitor.MediaList
import dna.familytree.visitor.NoteContainersGuarded
...
class MergeViewModel(state: SavedStateHandle) : ViewModel() {
    ...
    val firstTree: Settings.Tree
    lateinit var secondTree: Settings.Tree
    val trees = Global.settings.trees.filter { it.grade < 20 } as MutableList<Settings.Tree> // Derived and exhausted trees are excluded
    ...
    fun findMatches() {
        coroutine = viewModelScope.launch(Dispatchers.Default) {
            ...
            firstGedcom = TreesController.readJson(firstNum)
            if (isActive) secondGedcom = TreesController.readJson(secondNum.value!!) else return@launch
            matches.clear()
    ...
    private suspend fun copyMediaFiles(context: Context, sourceGedcom: Gedcom, sourceId: Int, destinationId: Int) {
        ...
        for (media in mediaList.list) {
            val path = dna.familytree.util.FileUtils.mediaPath(sourceId, media)
        ...
        if (mediaPaths.isNotEmpty()) {
            ...
            for (entry in mediaPaths.entries.iterator()) {
                ...
                val destinationFile = dna.familytree.util.FileUtils.nextAvailableFileName(
    ...
    private fun findMaxId(record: ExtensionContainer) {
        ...
        try {
            ...
            val num = AppUtils.extractNum(id)
    ...
    private fun isEligible(record: ExtensionContainer): Boolean {
        ...
        if (newId == null) {
            ...
            if (maxNum >= AppUtils.extractNum(oldId)) {
                maxNum++
    ...
    fun performAnnexMerge(context: Context) {
        coroutine = viewModelScope.launch(Dispatchers.Default) {
            ...
            doMerge()
            if (isActive) AppUtils.saveJson(firstGedcom, firstNum) // Saves also Global.settings through Notifier
    ...
    */
    fun performGenerateMerge(context: Context, title: String) {
        coroutine = viewModelScope.launch(Dispatchers.Default) {
            ...
            Global.settings.addTree(Settings.Tree(newNum, title, null, persons, generations, firstTree.root, null, 0))
            copyMediaFiles(context, firstGedcom, firstNum, newNum)
            copyMediaFiles(context, secondGedcom, secondNum.value!!, newNum)
            doMerge()
            if (isActive) AppUtils.saveJson(firstGedcom, newNum)
            setState(if (isActive) State.COMPLETE else State.QUIET)
        }
    }
    ...
    class Match(val left: Person, val right: Person) {
        ...
        override fun toString(): String {
            return "${AppUtils.properName(left)}, ${AppUtils.properName(right)}, $destiny"
        }
    }
}
